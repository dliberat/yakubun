...
function runRegex(target, rules, oAccumulator){
    ...
    var re = new RegExp('([0-9]+)([a-z][a-z]?)(?:\\W|$)','gi');
    
    var arr = general.regexReturnAllMatches(target, re);
    /* arr contains
...
function compareWithRules(arr, rules, oAccumulator){
    ...
    for(var i = 0; i < arr.length; i++){
        var strNum = arr[i][1].toString();
        var num = strNum.substr(strNum.length - 1);
...
function output(flag, oAccumulator){
    ...
    var retval = flag ? 'Check ordinal numbers.' : null;
