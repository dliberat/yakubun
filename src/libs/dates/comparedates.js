...
function convertToTwoDigitDates(comparison) {
  const digitAdder = function (match, m1, month, m2, date, m3, hour, m4) {
  ...
  const digitAdderNoTime = function (match, m1, month, m2, date, m3) {
    if (date.length == 1) { date = `0${date}`; }
    if (month.length == 1) { month = `0${month}`; }
    return m1 + month + m2 + date + m3;
  };

  const monthConverter = function (element, index) {
    // var re;
...
function convertToMomentArr(comparison, sourceTz, targetTz) {
  const newArr = [];
  const momentDateSort = function (a, b) {
  ...
  const sourceConverter = function (element) {
  ...
  const targetConverter = function (element) {
    element = element.slice(1, element.length - 1);
    if (targetTz) {
...
function removeMatchedDates(momentArr, sideArr, sideArrTarget) {
  ...
  let outFormatSourceArr;
  let outFormatTargetArr;
  outFormatSourceArr = momentArr[0].map(x => x.format('MMM. D'));
  outFormatTargetArr = momentArr[1].map(x => x.format('MMM. D'));
...
function removeSideArrStrings(sideArrStrings, clean_source_string) {
  ...
  // side string
  for (let i = 0; i < sideArrStrings.length; i++) {
...
function convertToISOTime(string) {
  const twentyFourHours = function (match, hr, min, mer) {
    hr = Number(hr);
    min = Number(min);
    mer = mer.toLowerCase();
  ...
  const newString = string.replace(/([0-2]?[0-9]):([0-5][0-9])([ap]m)/gi, twentyFourHours);
  return newString;
}
